...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Buscar os recursos para verificar permissões e obter URLs dos arquivos
    const { data: resources, error: resourceError } = await supabase
      .from('resources')
      .select('id, file_url, thumbnail_url, preview_url, creator_id')
      .in('id', resourceIds)
    ...
    // Verificar se o usuário é admin ou criador de todos os recursos
    ...
    // Filtrar recursos que o usuário pode deletar
    const deletableResources = resources.filter(resource => 
      isAdmin || resource.creator_id === user.id
    )

    if (deletableResources.length === 0) {
      return NextResponse.json({ 
        error: 'Você não tem permissão para deletar esses recursos' 
      }, { status: 403 })
    }
